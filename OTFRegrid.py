...
from LMTOTFFile import LMTOTFNetCDFFile
from collections import OrderedDict


from netCDF4 import Dataset, Variable, _private_atts
from dreampy.lmtnetcdf import pynetcdf4

#this part is temporary because the files are in a different directory
import sys
sys.path.insert(0, '/home/tkareta/summer/old_data/OTF/')

class LMTOTFRegrid(object):
    def __init__(self):
        self.get_parameters(525.0, 535.0, 645.0, 655.0)
        self.make_weights(1)
        self.get_reduc_prefs(True)
        self.make_filelist()
        self.make_arrays()
        #self.make_grid()
        #self.create_netcdf()
    def make_filelist(self):
        # when I figure out how we're going to input
        # the files into this class
        # e.g. an OrderedDict or a list or a tuple..
        # this will take it and make it into a specified
        # format  - for now, I've made a temporary list

        #one thing already settled is that the inputed
        #files will be ___.nc files
        
        filelist = ['35065305.nc','35065304.nc','35065306.nc','35065300.nc','35065301.nc','35065302.nc','35065303.nc','35065307.nc','35065308.nc','35065309.nc']
        setattr(self, 'filelist', filelist)
        temp_otfscan = LMTOTFNetCDFFile(filelist[0])
        setattr(self, 'nchan', temp_otfscan.hdu.header.nchan)
    
    def get_parameters(self, xmin, xmax, ymin, ymax, theta_n = 25.0, tsysweight=False, sigmaweight=True):
        # this assigns some temporary values
        # to xmax, xmin, ymax, ymin and theta_n
        # which are the parameters specified by
        # the user - (this is to come)
        # more descriptions below in self.make_arrays()
        setattr(self, 'xmin', xmin)
        setattr(self, 'xmax', xmax)
        setattr(self, 'ymin', ymin)
        setattr(self, 'ymax', ymax)
        setattr(self, 'theta_n', theta_n)
        setattr(self, 'RMAX', 3.0)
        #this changes the weighting in the
        #regrid method to be temperature dependent
        setattr(self, 'tsysweight', tsysweight)
        setattr(self, 'sigmaweight', sigmaweight)
        
        # this part is in SetFITSHEADERS.c - I don't know if it is actually
        # how crval2, crval3 are assigned
    ...
    def get_reduc_prefs(self, biased):
